...
// 3. Integrated with the new PageLayout component for consistent page structure and mobile responsiveness.
// 4. **FIXED:** Replaced problematic inline SVG with FaLink from react-icons/fa6.

import React, { useState, useCallback } from 'react';
import PageLayout from '../components/PageLayout'; // Import the new PageLayout component
import { FaLink } from 'react-icons/fa6'; // Import FaLink for external link icon
...
const SocialMediaHubPage = ({ db, firestoreAppId }) => {
  ...
  const generateContent = useCallback(async () => {
    ...
    try {
      ...
      const apiKey = ""; // Keep this as an empty string; Canvas will provide it at runtime
  ...
  return (
    ...
    <PageLayout pageTitle="Social Media Hub">
      {/* The main content for the Social Media Hub page, now inside PageLayout */}
      {/* Removed w-full, h-full, flex-col, min-w-0 from here as PageLayout provides it */}
      <div className="text-offWhite">
        ...
        <div className="bg-darkGray p-6 rounded-xl shadow-lg border border-gray-700 mb-8 text-center">
          ...
          >
            Go to Adobe Express
            {/* Replaced inline SVG with FaLink icon */}
            <FaLink className="h-5 w-5 ml-2" />
          </a>
        </div>
